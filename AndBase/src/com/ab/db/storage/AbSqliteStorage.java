...
public class AbSqliteStorage {
	 ...
	 *            返回监听器
	 */
	public <T> void insertData(final T entity, final AbDBDaoImpl<T> dao,
				...
				public <T> T getObject() {
					...
					try {
						...
						if (paramDataInsertListener != null) {
							paramDataInsertListener.onFailure(
									AbDBStatus.EXEC_ERROR_CODE, e.getMessage());
						}
					} finally {
						// (3)关闭数据库
						dao.closeDatabase();
				...
				public <T> T getObject() {
					...
					try {
						...
						if (paramDataInsertListListener != null) {
							paramDataInsertListListener.onFailure(
									AbDBStatus.EXEC_ERROR_CODE, e.getMessage());
						}
					} finally {
						// (3)关闭数据库
	 ...
	 *            返回监听器
	 */
	public <T> void findData(final AbStorageQuery storageQuery,
			final AbDBDaoImpl<T> dao,
			...
			public List<?> getList() {
				...
				try {
					...
					if (storageQuery.getLimit() != -1
						...
						list = dao.queryList(null,
								storageQuery.getWhereClause(),
								storageQuery.getWhereArgs(),
								storageQuery.getGroupBy(),
								storageQuery.getHaving(),
								storageQuery.getOrderBy() + " limit "
										+ storageQuery.getLimit() + " offset "
										+ storageQuery.getOffset(), null);
					} else {
						list = dao.queryList(null,
								storageQuery.getWhereClause(),
								storageQuery.getWhereArgs(),
					...
					if (paramDataSelectListener != null) {
						paramDataSelectListener.onFailure(
								AbDBStatus.EXEC_ERROR_CODE, e.getMessage());
					}
				} finally {
					// (3)关闭数据库
	 ...
	 *            返回监听器
	 */
	public <T> void updateData(final T entity, final AbDBDaoImpl<T> dao,
				...
				public <T> T getObject() {
					...
					try {
						...
						if (paramDataUpdateListener != null) {
							paramDataUpdateListener.onFailure(
									AbDBStatus.EXEC_ERROR_CODE, e.getMessage());
						}
					} finally {
						// (3)关闭数据库
	 ...
	 *            返回监听器
	 */
	public <T> void deleteData(final AbStorageQuery storageQuery,
			final AbDBDaoImpl<T> dao,
		...
		item.setListener(new AbTaskObjectListener() {
			...
			public <T> T getObject() {
				...
				try {
					...
					dao.startWritableDatabase(false);
					// (2)执行
					rows = dao.delete(storageQuery.getWhereClause(),
							storageQuery.getWhereArgs());
				} catch (Exception e) {
					e.printStackTrace();
					if (paramDataDeleteListener != null) {
						paramDataDeleteListener.onFailure(
								AbDBStatus.EXEC_ERROR_CODE, e.getMessage());
					}
				} finally {
					// (3)关闭数据库
