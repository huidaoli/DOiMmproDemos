...
public class ImageRequest extends Request<Bitmap> {
	...
	public ImageRequest(String url, Response.Listener<Bitmap> listener,
			...
			Response.ErrorListener errorListener, int expiresTime) {
	...
	private Response<Bitmap> doParse(NetworkResponse response) {
		...
		if (mMaxWidth == 0 && mMaxHeight == 0) {
			...
			if (tempBitmap != null
				...
				// 修改处理方式，解决OOM
				bitmap = AbImageUtil.getScaleBitmap(tempBitmap, desiredWidth,
						desiredHeight);
		...
		if (bitmap == null) {
			...
			return Response.success(bitmap,
					HttpHeaderParser.parseCacheHeaders(response, mExpiresTime));
		}
	}
