...
public abstract class XYChart extends AbstractChart {
				...
				for (int k = 0; k < yLimitArray.length; k++) {
					...
					if (k + 1 < yLimitArray.length) {
						...
						if (color == color2) {
							...
							canvas.drawLine(left, yLabel2, right, yLabel2,
									paint);
						}
						k++;
					} else {
		...
		for (int i = 0; i < sLength; i++) {
			...
			for (Entry<Double, Double> value : range.entrySet()) {
				...
				if (!isNullValue(yValue)) {
					pointsIndex.add((float) (left + xPixelsPerUnit[scale]
							* (xValue - minX[scale])));
					pointsIndex.add((float) (bottom - yPixelsPerUnit[scale]
							* (yValue - minY[scale])));
					...
					if (pointsIndex.size() > 0) {
						drawSeries(series, canvas, paint, pointsIndex,
								colorsIndex, seriesRenderer, yAxisValue, i, or,
								startIndex);
						ClickableArea[] clickableAreasForSubSeries = clickableAreasForPoints(
								MathHelper.getFloats(pointsIndex),
								MathHelper.getDoubles(valuesIndex), yAxisValue,
								i, startIndex);
			...
			if (points.size() > 0) {
				drawSeries(series, canvas, paint, pointsIndex, colorsIndex,
						seriesRenderer, yAxisValue, i, or, startIndex);
				ClickableArea[] clickableAreasForSubSeries = clickableAreasForPoints(
						MathHelper.getFloats(pointsIndex),
						MathHelper.getDoubles(valuesIndex), yAxisValue, i,
						startIndex);
	...
	protected void drawSeries(XYSeries series, Canvas canvas, Paint paint,
			List<Float> pointsList, List<Integer> colorsList,
		...
		drawSeries(canvas, paint, points, colors, seriesRenderer, yAxisValue,
				seriesIndex, startIndex);
		if (isRenderPoints(seriesRenderer)) {
			ScatterChart pointsChart = getPointsChart();
			if (pointsChart != null) {
				pointsChart.drawSeries(canvas, paint, points, colors,
	...
	/**
	 * The graphical representation of the series values as text. 绘制点上的文本
			...
			for (int k = 0; k < points.length; k += 2) {
				if (k == 2) { // decide whether to display first two points'
					...
					if (Math.abs(points[2] - points[0]) > 100
							|| Math.abs(points[3] - points[1]) > 100) {
					...
					if (Math.abs(points[k] - previousPointX) > 100
							|| Math.abs(points[k + 1] - previousPointY) > 100) {
						if (mRenderer.isDisplayValue0()) {
							drawText(
									canvas,
	...
	public abstract void drawSeries(Canvas canvas, Paint paint, float[] points,
			int[] colors, SimpleSeriesRenderer seriesRenderer,
