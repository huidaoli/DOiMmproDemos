...
public class AbTextView extends TextView {
	...
	protected void onDraw(Canvas canvas) {
		...
		drawText(canvas, this.getText().toString(), this.getWidth(),
				this.getPaint());
	}
	...
	public int subStringLength(String str, int maxPix, TextPaint paint) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			float valueLength = paint.measureText(temp) + leftPadding
					+ rightPadding;
	...
	public int drawText(Canvas canvas, String text, int maxWPix, TextPaint paint) {
		...
		for (int i = 0; i < mStrList.size(); i++) {
			// 计算坐标
			float x = leftPadding;
			float y = topPadding + hSize / 2 + i * (hSize + lineSpacing)
					+ bottomPadding;
	...
	public void setTextSize(float size) {
		...
		setRawTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
