...
public class AbLevelChart extends AbLevelAbstractChart {
	 ...
	 *            the m renderer
	 */
	public AbLevelChart(AbLevelSeriesDataset mDataset,
	 ...
	 *            the renderer
	 */
	protected void setDatasetRenderer(AbLevelSeriesDataset dataset,
	 ...
	 * @param paint
	 *            the paint
	 * @see com.ab.view.level.AbLevelAbstractChart#draw(android.graphics.Canvas,
	 *      int, int, int, int, int, int, android.graphics.Paint)
	 */
	public void draw(Canvas canvas, int x, int y, int measureWidth,
		...
		textLevelSize = AbViewUtil.scale(screenWidth, screenHeight,
				textLevelSize);
		// 当前值文字与顶部的距离
		...
		int textWidth = (int) AbGraphicUtil.getStringWidth(textValue,
				mTextPaint);
		...
		RectF mLevelRect = new RectF(left, topLevel, left + measureWidth,
				topLevel + levelHeight);
		...
		for (int i = 0; i < color.length; i++) {
			if (i == 0) {
				...
				mLevelRectPart = new RectF(sumLeft, topLevel, sumRight,
						topLevel + levelHeight);
			} else {
				sumLeft += part[i - 1] * partWidth;
				sumRight += part[i] * partWidth;
				mLevelRectPart = new RectF(sumLeft + 1, topLevel, sumRight,
						topLevel + levelHeight);
			}
			...
			if (textlevelIndex == i) {
				...
				canvas.drawText(textValue, sumLeft + textLeftOffset, marginTop,
						paint);
				...
				path1.lineTo(sumLeft + arrowLeftOffset + arrowWidth, marginTop
						+ textHeight);
				path1.close();
				...
				paint.setStyle(Paint.Style.FILL);
				Path path2 = new Path();
				path2.moveTo(center, marginTop + textHeight + arrowHeight
						+ levelHeight + 2 * arrowMarginTop);
				path2.lineTo(sumLeft + arrowLeftOffset, marginTop + textHeight
						+ levelHeight + 2 * arrowHeight + 2 * arrowMarginTop);
				path2.lineTo(sumLeft + arrowLeftOffset + arrowWidth, marginTop
						+ textHeight + levelHeight + 2 * arrowHeight + 2
						* arrowMarginTop);
				path2.close();
				canvas.drawPath(path2, paint);

				// 绘制等级条下方的文字描述
				int topDesc = marginTop + textHeight + 2 * arrowHeight + 2
						* arrowMarginTop + levelHeight;
				RectF mLevelDescRect = new RectF(center
						- mRenderer.getTextRectWidth() / 2, topDesc, center
						+ mRenderer.getTextRectWidth() / 2, topDesc
						+ mRenderer.getTextRectHeight());
				...
				// 得到行高
				int textDescHeight = (int) Math.ceil(textDescFm.descent
						- textDescFm.ascent) + 2;
				int textDescWidth = (int) AbGraphicUtil.getStringWidth(
						textDesc, mTextPaint);
				canvas.drawText(
						textDesc,
						center - textDescWidth / 2,
						topDesc
								+ 20
								+ ((mRenderer.getTextRectHeight() - textDescHeight) / 2),
						paint);
				paint.setColor(color[i]);
			...
			if (partValue != null && partValue.length == color.length) {
				...
				int partValueWidth = (int) AbGraphicUtil.getStringWidth(
						String.valueOf(partValue[i]), mTextPaint);
				canvas.drawText(String.valueOf(partValue[i]),
						mLevelRectPart.left - partValueWidth / 2,
						mLevelRectPart.top + levelHeight + 15, paint);
			}
