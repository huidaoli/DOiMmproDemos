...
/**
 * © 2012 amsoft.cn 名称：CarouselView.java
 * 
 */
public class CarouselView extends CarouselSpinner implements
	...
	protected int getChildDrawingOrder(int childCount, int i) {
		...
		for (int j = 0; j < childCount; j++) {
			CarouselItemView view = (CarouselItemView) getAdapter().getView(j,
					null, null);
			if (i == 0)
				view.setDrawn(false);
	...
	protected boolean getChildStaticTransformation(View child,
		...
		mCamera.translate(((CarouselItemView) child).getItemX(),
	 ...
	 *            the angle offset
	 */
	private void Calculate3DPosition(CarouselItemView child, int diameter,
			float angleOffset) {
	...
	private int getCenterOfGallery() {
		return (getWidth() - CarouselView.this.getPaddingLeft() - CarouselView.this
				.getPaddingRight()) / 2 + CarouselView.this.getPaddingLeft();
	}
	...
	float getLimitedMotionScrollAmount(boolean motionToLeft, float deltaX) {
		int extremeItemPosition = motionToLeft ? CarouselView.this.getCount() - 1
				: 0;
		View extremeChild = getChildAt(extremeItemPosition
				- CarouselView.this.getFirstVisiblePosition());
	...
	private void makeAndAddView(int position, float angleOffset) {
		...
		if (!mDataChanged) {
			...
			if (child != null) {
				...
				child = (CarouselItemView) mAdapter.getView(position, null,
						this);
	...
	void scrollToChild(int i) {
		...
		CarouselItemView view = (CarouselItemView) getAdapter().getView(i,
				null, null);
	...
	void trackMotionScroll(float deltaAngle) {
		...
		for (int i = 0; i < getAdapter().getCount(); i++) {
			...
			CarouselItemView child = (CarouselItemView) getAdapter().getView(i,
					null, null);
