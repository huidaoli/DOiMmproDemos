 ...
 * 
 */
public class CarouselImageView extends CarouselSpinner implements
	...
	protected int getChildDrawingOrder(int childCount, int i) {
		...
		for (int j = 0; j < childCount; j++) {
			CarouselItemImage view = (CarouselItemImage) getAdapter().getView(
					j, null, null);
			if (i == 0)
				view.setDrawn(false);
	...
	protected boolean getChildStaticTransformation(View child,
		...
		mCamera.translate(((CarouselItemImage) child).getItemX(),
				((CarouselItemImage) child).getItemY(),
	...
	public boolean onDown(MotionEvent e) {
		...
		mDownTouchPosition = pointToPositionImage((int) e.getX(),
				(int) e.getY());
	 ...
	 *            the angle offset
	 */
	private void Calculate3DPosition(CarouselItemImage child, int diameter,
			float angleOffset) {

		angleOffset = angleOffset * (float) (Math.PI / 180.0f);

		float x = -(float) (diameter / 2 * android.util.FloatMath
				.sin(angleOffset)) + diameter / 2 - child.getWidth() / 2;
		float z = diameter / 2
				* (1.0f - (float) android.util.FloatMath.cos(angleOffset));
		float y = -getHeight() / 2
				+ (float) (z * android.util.FloatMath.sin(mTheta));
	...
	private int getCenterOfGallery() {
		return (getWidth() - CarouselImageView.this.getPaddingLeft() - CarouselImageView.this
				.getPaddingRight())
				/ 2
	...
	float getLimitedMotionScrollAmount(boolean motionToLeft, float deltaX) {
		int extremeItemPosition = motionToLeft ? CarouselImageView.this
				.getCount() - 1 : 0;
		View extremeChild = getChildAt(extremeItemPosition
				- CarouselImageView.this.getFirstVisiblePosition());
	...
	private void makeAndAddView(int position, float angleOffset) {
		...
		if (!mDataChanged) {
			...
			if (child != null) {
				...
				child = (CarouselItemImage) mAdapter.getView(position, null,
						this);
	...
	void scrollToChild(int i) {
		...
		CarouselItemImage view = (CarouselItemImage) getAdapter().getView(i,
				null, null);
	 ...
	 *            the angle offset
	 */
	private void setUpChild(CarouselItemImage child, int index,
			float angleOffset) {
	...
	void trackMotionScroll(float deltaAngle) {
		...
		for (int i = 0; i < getAdapter().getCount(); i++) {
			...
			CarouselItemImage child = (CarouselItemImage) getAdapter().getView(
					i, null, null);
