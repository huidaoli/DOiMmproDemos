...
public class AbSlidingSmoothTabView extends LinearLayout {
	...
	public AbSlidingSmoothTabView(Context context, AttributeSet attrs) {
		...
		if (!(this.context instanceof FragmentActivity)) {
			AbLogUtil
					.e(AbSlidingSmoothTabView.class,
							"构造AbSlidingSmoothTabView的参数context,必须是FragmentActivity的实例。");
		}
		...
		mTabScrollView.setOnScrollListener(new AbOnScrollListener() {
			...
			public void onScroll(int arg1) {
				...
				AbLogUtil.d(AbSlidingSmoothTabView.class, "滑动X" + startX + "to"
						+ toX);
	...
	public void computeTabImg(int index) {
		...
		AbLogUtil.d(AbSlidingSmoothTabView.class, "view宽度" + index + ":"
				+ tabView.getMeasuredWidth());
		AbLogUtil.d(AbSlidingSmoothTabView.class, "ScrollView宽度:"
				+ mTabScrollView.getWidth());
		AbLogUtil.d(AbSlidingSmoothTabView.class, "scrollX:" + scrollX);
		AbLogUtil.d(AbSlidingSmoothTabView.class,
				"tabView right:" + tabView.getRight());
		AbLogUtil.d(AbSlidingSmoothTabView.class,
		...
		if (mSelectedTabIndex < index
			...
			if (index == mTabLayout.getChildCount() - 1) {
				...
				AbLogUtil.d(AbSlidingSmoothTabView.class, "startX:" + startX
						+ ",offsetX:" + offsetX);
			...
			if (index == 0) {
				...
				mTabScrollView.smoothScrollBy(offsetX, 0);
				scrollX = scrollX + offsetX;
				AbLogUtil.d(AbSlidingSmoothTabView.class, "startX2:" + startX
						+ ",offsetX:" + offsetX);
				int toX = tabView.getLeft() - scrollX;
				imageSlide(mTabImg, startX, toX, 0, 0);
				startX = toX;
