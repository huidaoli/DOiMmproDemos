...
public class JsonReader implements Closeable {
	...
	private void expect(JsonToken expected) throws IOException {
		...
		if (token != expected) {
			throw new IllegalStateException("Expected " + expected
					+ " but was " + peek() + " at line " + getLineNumber()
	...
	public boolean nextBoolean() throws IOException {
		...
		if (token != JsonToken.BOOLEAN) {
			throw new IllegalStateException("Expected a boolean but was "
					+ token + " at line " + getLineNumber() + " column "
					+ getColumnNumber());
		}
	...
	public double nextDouble() throws IOException {
		...
		if ((result >= 1.0d && value.startsWith("0"))) {
			throw new MalformedJsonException("JSON forbids octal prefixes: "
					+ value + " at line " + getLineNumber() + " column "
					+ getColumnNumber());
		}
		if (!lenient && (Double.isNaN(result) || Double.isInfinite(result))) {
			throw new MalformedJsonException(
					"JSON forbids NaN and infinities: " + value + " at line "
	 ...
	 *            true for this method to only set the valuePos and valueLength
	 *            fields and return a null result. This only works if the
