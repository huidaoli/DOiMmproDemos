...
final class TreeTypeAdapter<T> extends TypeAdapter<T> {
	...
	public T read(JsonReader in) throws IOException {
		...
		return deserializer.deserialize(value, typeToken.getType(),
	...
	public void write(JsonWriter out, T value) throws IOException {
		...
		JsonElement tree = serializer.serialize(value, typeToken.getType(),
				gson.serializationContext);
		Streams.write(tree, out);
	}

	private TypeAdapter<T> delegate() {
		TypeAdapter<T> d = delegate;
		return d != null ? d : (delegate = GsonInternalAccess.INSTANCE
				.getNextAdapter(gson, skipPast, typeToken));
	}
	...
	private static class SingleTypeFactory implements TypeAdapterFactory {
		...
		private SingleTypeFactory(Object typeAdapter, TypeToken<?> exactType,
			...
			serializer = typeAdapter instanceof JsonSerializer ? (JsonSerializer<?>) typeAdapter
					: null;
			deserializer = typeAdapter instanceof JsonDeserializer ? (JsonDeserializer<?>) typeAdapter
					: null;
			$Gson$Preconditions.checkArgument(serializer != null
					|| deserializer != null);
		...
		public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
			boolean matches = exactType != null ? exactType.equals(type)
