...
import com.can2do.doimmpro.demo.adapter.ImageGridAdapter;
...
public class PullToRefreshGridActivity extends AbActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		 ...
		 * //得到一个LayoutAnimationController对象; LayoutAnimationController lac =
		 * new LayoutAnimationController(animationAlpha); //设置控件显示的顺序;
		 * lac.setOrder(LayoutAnimationController.ORDER_RANDOM); //设置控件显示间隔时间;
		 * lac.setDelay(0.5f); //为View设置LayoutAnimationController属性;
		 * mGridView.setLayoutAnimation(lac);
		 */
		...
		mGridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				AbToastUtil.showToast(PullToRefreshGridActivity.this, ""
						+ position);
			}
			...
			public void get() {
				try {
					...
					AbToastUtil.showToastInThread(
							PullToRefreshGridActivity.this, e.getMessage());
				}
			};
		});
		...
		item.setListener(new AbTaskListener() {
			...
			public void get() {
				try {
					...
					for (int i = 0; i < pageSize; i++) {
						final User mUser = new User();
						mUser.setHeadUrl(mPhotoList.get(new Random()
