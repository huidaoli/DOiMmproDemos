...
import com.can2do.doimmpro.demo.adapter.ImageShowAdapter;
...
public class PHashActivity extends AbActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		createBtn.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				AbDialogUtil.showProgressDialog(PHashActivity.this,
						R.drawable.progress_circular, "正在查询...");
					...
					public void get() {
						try {
							...
							if (!Environment.MEDIA_MOUNTED.equals(Environment
								...
								// 有sd卡
								File pathSD = Environment
										.getExternalStorageDirectory();
								File fileDirectory = new File(pathSD
										.getAbsolutePath()
										+ File.separator
										+ "DCIM" + File.separator);
							...
							for (int i = 0; i < files.size(); i++) {
								File f = files.get(i);
								AbLogUtil.d(PHashActivity.this,
										"图片的路径是 = " + f.getPath());
								...
								if (bitmap == null) {
									...
									// 计算hash
									String hashCode = AbImageUtil
											.getDCTHashCode(bitmap);
									hashCodes.add(hashCode);
									AbLogUtil.d(PHashActivity.this,
											"hashCodes add:" + i + ":"
													+ hashCode);
									// 颜色分布
									Bitmap bitmapT = AbImageUtil.getCutBitmap(
											bitmap, 360, 360);
									int[] colors = AbImageUtil
											.getColorHistogram(bitmapT);
							...
							AbToastUtil.showToastInThread(PHashActivity.this,
									e.getMessage());
						}
					};
				});
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		case CAMERA_WITH_DATA:
			AbLogUtil.d(PHashActivity.this, "将要进行裁剪的图片的路径是 = "
		...
		case CAMERA_CROP_DATA:
			...
			AbLogUtil.d(PHashActivity.this, "this image sourceHashCode:"
					+ sourceHashCode);
			// 计算距离
			for (int i = 0; i < hashCodes.size(); i++) {
				String hashCode = hashCodes.get(i);
				int distanceNew = AbImageUtil.hammingDistance(sourceHashCode,
						hashCode);
				if (distanceNew < 5) {
			...
			Map.Entry[] hashCodesAndDisNew = (Map.Entry[]) set
			...
			for (int i = 0; i < hashCodesAndDisNew.length; i++) {
				String pathNew = files.get(
						(Integer) hashCodesAndDisNew[i].getKey()).getPath();
				AbLogUtil.d(PHashActivity.this, "匹配结果:" + pathNew + ":"
						+ hashCodesAndDisNew[i].getValue());
				mImagePathAdapter
						.addItem(mImagePathAdapter.getCount(), pathNew);
	...
	public void createChart(int[] colorsArea, String title) {
		...
		LinearLayout chartView = (LinearLayout) mInflater.inflate(
				R.layout.chart_view_item, null);
		...
		// int[] colorsLimit = new int[] { Color.rgb(100,
		// 255,255),Color.rgb(100, 255,255),Color.rgb(0, 255, 255),Color.rgb(0,
		// 255, 255) };
		...
		// 线图
		View chart = ChartFactory.getAreaChartView(this,
				mXYMultipleSeriesDataset, renderer, 0.2f);
