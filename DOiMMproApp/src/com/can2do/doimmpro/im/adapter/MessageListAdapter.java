package com.can2do.doimmpro.im.adapter;
...
/**
 * © 2012 amsoft.cn 名称：MessageListAdapter 描述：消息列表
 * 
 * @version
 */
public class MessageListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = mInflater.inflate(R.layout.im_message_item, parent,
					false);
			// 减少findView的次数
			...
			holder.operateLayout = (LinearLayout) convertView
		...
		holder.acceptBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// 接受请求
				IMUtil.sendSubscribe(Presence.Type.subscribed,
						IMUtil.getJidByName(mMessage.getUserName()));
				IMUtil.sendSubscribe(Presence.Type.subscribe,
						mMessage.getUserName());
				holder.operateLayout.setVisibility(View.GONE);
			}
		});
		...
		holder.rejectBtn.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				IMUtil.sendSubscribe(Presence.Type.unsubscribe,
						IMUtil.getJidByName(mMessage.getUserName()));
				holder.operateLayout.setVisibility(View.GONE);
			}
		});
