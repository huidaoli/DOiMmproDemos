...
import com.can2do.doimmpro.im.adapter.ChatMsgViewAdapter;
import com.can2do.doimmpro.im.dao.IMMsgDao;
import com.can2do.doimmpro.im.global.IMConstant;
import com.can2do.doimmpro.im.model.IMMessage;
import com.can2do.doimmpro.im.util.IMRecordListener;
import com.can2do.doimmpro.im.util.IMRecorder;
...
public class ChatActivity extends AbActivity {
	...
	private BroadcastReceiver mDataReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			IMMessage mIMMessage = (IMMessage) intent
					.getParcelableExtra("MESSAGE");

			if (mIMMessage != null) {
				if (D)
					Log.d(TAG, "收到了消息:" + mIMMessage.getContent());
				mIMMessage.setSendState(IMMessage.READ);
				if (application.mUser != null) {
					if (mIMMessage.getToUserName().equals(
							application.mUser.getUserName())) {
	...
	public void saveMessageData(final IMMessage chatMsg) {
		mAbSqliteStorage.insertData(chatMsg, mIMMsgDao,
	...
	public void queryMsgList() {
		...
		mAbStorageQuery2
				.equals("to_user_name", application.mUser.getUserName());
	...
	public void updateData(IMMessage mChatMsg) {
		...
		mAbSqliteStorage.updateData(mChatMsg, mIMMsgDao,
				new AbDataUpdateListener() {
			...
			mIMRecorder = new IMRecorder(this, new IMRecordListener() {
				...
				public void onFinish(File file, long time) {
					AbDialogUtil.showAlertDialog(
							ChatActivity.this,
							"录音完成",
							"录音完成,文件在" + file.getPath() + ",长度为："
									+ AbDateUtil.getTimeDescription(time));
				}

				@Override
				public void onError(int errorCode, String errorMessage) {
					AbToastUtil.showToast(ChatActivity.this, "提示："
							+ errorMessage);
				}
